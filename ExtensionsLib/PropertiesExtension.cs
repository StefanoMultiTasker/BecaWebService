...
namespace ExtensionsLib
{
    public static partial class PropertiesExtension
    {
        ...
        {
            Type type = @this.GetType();
            string realName = type.GetProperties().FirstOrDefault(p => p.Name.ToLower() == propertyName.ToLower()).Name;
            PropertyInfo property = type.GetProperty(realName, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static);
